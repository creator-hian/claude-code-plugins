...
Register this plugin to marketplace.json? (y/n): y
```
...
### 자동 등록 (create_plugin.py 사용)

스크립트 실행 시 선택적으로 자동 등록 가능:
...
`marketplace.json` 파일을 직접 편집:

```json
{
  "name": "creator-hian-marketplace",
  "owner": {
    "name": "Creator Hian"
  },
  "plugins": [
