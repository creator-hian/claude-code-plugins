...
```bash
# Maximum depth: gpt-5.3-codex (best) + xhigh
...
# Fast: mini + low
codex exec -m gpt-5-codex-mini -c model_reasoning_effort=low -s read-only \
  "Quick syntax check"
```
...
```bash
# Vulnerability scan
codex exec -m gpt-5.3-codex -c model_reasoning_effort=high -s read-only \
...
codex exec --skip-git-repo-check -m gpt-5.3-codex -s read-only "Review code"
```
